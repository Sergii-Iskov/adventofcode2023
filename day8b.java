 ...
 * Calculation by "least common multiple" (findNSK and findNSD created by ChatGPT)
 * Created by Sergii Iskov, 2023. Version 2.1
 */
public class day8b {
    public static void main(String[] args) {
        String[] dataArr = data.split("\n");
        HashMap<String, String[]> dataMap = fillMap(dataArr);
        System.out.println("instruction start = " + instruction.length());

        int[] resultArr = new int[6];
        int count = 0;
        for (String key : dataMap.keySet()) {
            if (key.charAt(2) == 'A') {
                resultArr[count] = findRoad(key, dataMap);
                System.out.println("key : " + resultArr[count]);
                count++;
            }
        }
        ...
        for (int i = 0; i < resultArr.length - 1; i++) {
            long a = resultArr[i];
            long b = resultArr[i + 1];
            step = (step == 0) ? findNSK(a, b) : findNSK(step, b);
        }
        System.out.println(step);
    }

    // Method for calculation "least common multiple"
    public static long findNSK (long a, long b) {
        long nsd = findNSD(a, b);
        // NSK(a, b) = |a * b| / NSD(a, b)|
        return Math.abs(a * b) / nsd;
    }

    // Method for calculation "lowest common denominator" (Euclid`s algorithm)
    private static long findNSD (long a, long b) {
        while (b != 0) {
            long temp = b;
            b = a % b;
            a = temp;
    ...
    // Method for calculation min steps from "**A" to "**Z"
    private static int findRoad(String s, HashMap<String, String[]> map) {
        String currentPosition = s;
        int step = 0;
//        System.out.print(currentPosition + " - ");
        for (int i = 0; i < instruction.length(); i++) {
            char ch = instruction.charAt(i);
            step++;
            if (ch == 'L') currentPosition = map.get(currentPosition)[0];
            if (ch == 'R') currentPosition = map.get(currentPosition)[1];
//            System.out.print(currentPosition + " - ");
            if (currentPosition.charAt(2) == 'Z') return step;
            if (i == instruction.length() - 1) instruction += instruction;
        }
        return step;
    }
