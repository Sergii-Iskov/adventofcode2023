...
/**
 * This task "Day 9: Pipe Maze" is from https://adventofcode.com/2023/day/10
 * We need to find How many tiles are enclosed by the loop.
...
public class day10b {
    ...
    public static void main(String[] args) {
        ...
        int[][] ar = bfs(coordinates, maze);
        System.out.println("plate = " + findTiles(maze, ar));
    }

    private static int[][] bfs(ArrayList<int[]> coordinates, String[][] maze) {
        int length = maze.length;
        int length1 = maze[0].length;
        int[][] ar = new int[length][length1];            // 0 - unvisited; 1 - loop; 2 - visited empty
        ...
        for (int[] coordinate : coordinates) {
            ar[coordinate[1]][coordinate[0]] = 1;
        }
        ...
        while (!queue.isEmpty()) {
            ...
            if ((x >= 0 && x < length1 && y >= 0 && y < length) && ar[y][x] != 1 && ar[y][x] != 2) {
                ar[y][x] = 2;
                int[][] directions = {                         // all pixel direction, where {x, y}
    ...
    private static int findTiles(String[][] maze, int[][] ar) {
        int plateNumber = 0;
        for (int i = 0; i < ar.length; i++) {
            String newString = "";
            for (int j = 0; j < ar[0].length; j++) {
                if (ar[i][j] == 0) newString += "0";
                if (ar[i][j] == 1) newString += maze[i][j];
                if (ar[i][j] == 2) newString += " ";
            }
//            System.out.println(newString);   // uncomment to see string before transformed
            newString = newString.replace("-", "").replace("FS", "")
                    .replace("FJ", "1").replace("L7", "1").replace("|", "1")
                    .replace("F7", "").replace("LJ", "");
//            System.out.println(newString);   // uncomment to see string after transformed

            for (int j = 0; j < newString.length(); j++) {
                if (newString.charAt(j) == '0') {
                    int count = 0;
                    for (int k = 0; k < j; k++) {
                        if (newString.charAt(k) == '1') count += 1;
                    }
    ...
    private static void fistStep(String[][] maze, int x, int y) {
        ...
        if (isInMaze(maze, x, y - 1) && "\\|F7".contains(maze[y - 1][x])) key = "Up";
        else if (isInMaze(maze, x, y + 1) && "\\|LJ".contains(maze[y + 1][x])) key = "Down";
        else if (isInMaze(maze, x - 1, y) && "-FL".contains(maze[y][x - 1])) key = "Left";
        else if (isInMaze(maze, x + 1, y) && "7-J".contains(maze[y][x + 1])) key = "Right";
