...
/**
 * This task "Day 3: Gear Ratios" is from https://adventofcode.com/2023/day/3#part2
 * We need to calculate the sum of all of the gear ratios in your engine schematic.
...
public class day3b {
    ...
    public static void main(String[] args) {
        ...
        boolean[][] logicArr = new boolean[gameArray.length][gameArray[0].length()];

        for (int row = 0; row < gameArray.length; row++) {
            String currentCol = gameArray[row];
            for (int col = 0; col < currentCol.length(); col++) {
                char ch = currentCol.charAt(col);
                if (ch == '.') {                        // visited digital cells and sells with "." checked as true
                    logicArr[row][col] = true;
                } else if (ch == '*') {
                    int gearRatios = 0;
                    int number1 = 0, number2 = 0;
                    for (int x = -1; x < 2; x++) {
                        for (int y = -1; y < 2; y++) {
                            if (checkInBorder(logicArr, col + x, row + y) && !logicArr[row + y][col + x]) {
                                char currentCh = gameArray[row + y].charAt(col + x);
                                if (Character.isDigit(currentCh)) {
                                    String newNumber = "" + currentCh;
                                    logicArr[row + y][col + x] = true;
                                    // Check neighbors of current char is digit
                                    newNumber = findNeighbor(newNumber, gameArray, logicArr, row + y, col + x, -1);
                                    newNumber = findNeighbor(newNumber, gameArray, logicArr, row + y, col + x, 1);

                                    if (number1 == 0) {
                                        number1 = Integer.parseInt(newNumber);
                                    } else number2 = Integer.parseInt(newNumber);
                                    System.out.println("Row " + (row + 1) + ", char " + currentCh + ", number = " + newNumber);
                                }
                            }
                        }
                    }
                    ...
                    totalGearRatios += gearRatios;
                }
            }
        }
        System.out.println("Total count = " + totalGearRatios);
    }

    /**
     * Check neighbors of current char is digit and return number after correction
     * @param newNumber      number
     * @param gameArray      data array
     * @param logicArr boolean array
     * @param x      coordinate x of char
     * @param y      coordinate y of char
     * @param dir      direction of view (left = -1, right = +1)
     *
     * @return number after correction
     */
    private static String findNeighbor(String newNumber, String[] gameArray, boolean[][] logicArr, int y, int x, int dir) {
        if (checkInBorder(logicArr, x + dir, y) && !logicArr[y][x + dir]
                && Character.isDigit(gameArray[y].charAt(x + dir))) {
            logicArr[y][x + dir] = true;
            // construct number from digits
            newNumber = (dir > 0) ? ("" + newNumber + gameArray[y].charAt(x + dir))
                    : ("" + gameArray[y].charAt(x + dir) + newNumber);
            // start recursion - find new digits in chosen direction
            newNumber = (dir > 0) ? findNeighbor(newNumber, gameArray, logicArr, y, x + dir, 1)
                    : findNeighbor(newNumber, gameArray, logicArr, y, x + dir, -1);
        }
        return newNumber;
    }
     ...
     */
    private static boolean checkInBorder(boolean[][] logicArr, int x, int y) {
        return x >= 0 && y >= 0 && x <= logicArr[0].length - 1 && y <= logicArr.length - 1;
    }
